            ...
            <tr>
                ...
                <td>
                    <form action="{{ route('gestionartipousuario.destroy',$tipousuario->id) }}" method="POST"
                        ...
                        {{ method_field('DELETE') }}
                        @if(\App\DetalleTipoUsuarioCDU::where('idtipousuario','=',Auth::user()->idtipousuario)
                 ->where('idcdu','5')->first()->editar==1)
                        <a href="{{ route('gestionartipousuario.edit', $tipousuario->id) }}" class="btn btn-info btn-round">Editar</a>@endif
                        @if(\App\DetalleTipoUsuarioCDU::where('idtipousuario','=',Auth::user()->idtipousuario)
                 ->where('idcdu','5')->first()->eliminar==1)
